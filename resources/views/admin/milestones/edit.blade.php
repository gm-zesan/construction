...
@section('content')
    <div class="container-fluid my-3">
        <form id="milestoneEditForm" action="{{ route('milestones.update', $milestone->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                {{-- Main Form Details --}}
                    ...
                    <div class="card table-card mb-4">
                        <div class="card-header table-header">
                            <div class="title-with-breadcrumb">
                                <div class="table-title">Edit Project Milestone</div>
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb mb-0">
                                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                                        <li class="breadcrumb-item"><a href="{{ route('milestones.index') }}">Milestones</a></li>
                                        <li class="breadcrumb-item active" aria-current="page">Edit #{{ $milestone->id }}</li>
                                    </ol>
                                </nav>
                            </div>
                            <div class="d-flex gap-2">
                                <a href="{{ route('milestones.show', $milestone->id) }}" class="add-new" style="background-color: #f1f5f9; color: #334155;">
                        ...
                        <div class="card-body custom-form p-4">
                            <div class="row g-3">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select class="form-select custom-input @error('project_id') is-invalid @enderror" name="project_id" id="project_id" required>
                                        ...
                                        @foreach($projects as $p)
                                            <option value="{{ $p->id }}" {{ old('project_id', $milestone->project_id) == $p->id ? 'selected' : '' }}>
                                                {{ $p->title }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input type="text" class="form-control custom-input @error('title') is-invalid @enderror" name="title" id="title" value="{{ old('title', $milestone->title) }}" placeholder="e.g. Structural Steel Superstructure Complete" required>
                                    @error('title')
                                        <div class="text-danger mt-1" style="font-size: 12px;">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="date" class="form-control custom-input @error('target_date') is-invalid @enderror" name="target_date" id="target_date" value="{{ old('target_date', $milestone->target_date?->format('Y-m-d')) }}" required>
                                    @error('target_date')
                                        <div class="text-danger mt-1" style="font-size: 12px;">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="date" class="form-control custom-input @error('completion_date') is-invalid @enderror" name="completion_date" id="completion_date" value="{{ old('completion_date', $milestone->completion_date?->format('Y-m-d')) }}">
                                ...
                                <div class="col-md-12">
                                    <label for="progress_percentage" class="form-label custom-label d-flex justify-content-between">
                                        ...
                                        <span class="fw-bold text-primary" id="progressValBadge">{{ old('progress_percentage', $milestone->progress_percentage) }}%</span>
                                    </label>
                                    <input type="range" class="form-range" name="progress_percentage" id="progress_percentage" min="0" max="100" step="5" value="{{ old('progress_percentage', $milestone->progress_percentage) }}" oninput="$('#progressValBadge').text(this.value + '%');">
                                </div>

                                {{-- Description --}}
                                <div class="col-12">
                                    <label for="description" class="form-label custom-label">Phase Description &amp; Scope of Inspection</label>
                                    <textarea class="form-control custom-input @error('description') is-invalid @enderror" name="description" id="description" rows="4" placeholder="Detailed summary of work completed for this milestone...">{{ old('description', $milestone->description) }}</textarea>
                                    @error('description')
                                        <div class="text-danger mt-1" style="font-size: 12px;">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="card-body custom-form p-3">
                                    <div class="d-flex flex-column gap-3 mb-3">
                                        <div>
                                            ...
                                            <select class="form-select custom-input" name="status" id="status" required>
                                                @foreach($statuses as $st)
                                                    <option value="{{ $st->value }}" {{ old('status', $milestone->status instanceof \App\Enums\MilestoneStatus ? $milestone->status->value : $milestone->status) == $st->value ? 'selected' : '' }}>
                                                        {{ $st->label() }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" type="checkbox" name="is_published" id="is_published" value="1" {{ old('is_published', $milestone->is_published) ? 'checked' : '' }} style="cursor: pointer;">
                                        ...
                                        <div>
                                            ...
                                            <input type="number" class="form-control custom-input" name="sort_order" id="sort_order" value="{{ old('sort_order', $milestone->sort_order) }}" min="0">
                            ...
                            <div class="card table-card">
                                ...
                                <div class="card-body custom-form p-3">
                                    ...
                                    <div id="image_dropzone" class="dropzone-box">
                                        <div id="image_empty" class="{{ $milestone->hasMedia('image') ? 'd-none' : '' }}">
                                        ...
                                        <div id="image_preview_box" class="{{ $milestone->hasMedia('image') ? '' : 'd-none' }}">
                                            <div class="position-relative rounded overflow-hidden mb-2" style="height: 160px; background: #000;">
                                                <img id="image_preview_img" src="{{ $milestone->image_url ?? '' }}" alt="Verification preview"
                                                    onerror="this.onerror=null;this.src='{{ asset('admin/assets/images/default.jpg') }}';"
                                                    style="width: 100%; height: 100%; object-fit: cover;">
                                                <span class="badge-cover-type" id="image_badge" style="position: absolute; top: 8px; left: 8px; font-size: 10.5px; padding: 3px 7px; background: rgba(0,0,0,0.65); color: #fff; border-radius: 4px; backdrop-filter: blur(4px);">
                                                    <i class="ri-check-line me-1 text-success"></i> Current Photo
                                                </span>
                                                <button type="button" class="preview-remove-btn {{ $milestone->hasMedia('image') ? 'd-none' : '' }}" id="btn_remove_image" title="Cancel selected image">
                                                    <i class="ri-close-line"></i>
                                                </button>
                                            </div>
                                            <div class="d-flex align-items-center justify-content-between px-1">
                                                <div class="text-start text-truncate me-2">
                                                    <span id="image_preview_name" class="fw-semibold text-dark d-block text-truncate" style="font-size: 12px;">{{ $milestone->getFirstMedia('image')?->file_name ?? 'Current Photo' }}</span>
                                                    <span id="image_preview_size" class="text-muted" style="font-size: 11px;">{{ $milestone->getFirstMedia('image')?->readable_size ?? 'Active verification photo' }}</span>
                                                </div>
...
@push('custom-scripts')
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var originalPhotoSrc = "{{ $milestone->image_url ?? '' }}";
            var originalPhotoName = "{{ $milestone->getFirstMedia('image')?->file_name ?? 'Current Photo' }}";
            var originalPhotoSize = "{{ $milestone->getFirstMedia('image')?->readable_size ?? 'Active verification photo' }}";
            var hasOriginalImage = {{ $milestone->hasMedia('image') ? 'true' : 'false' }};
            ...
            function renderPreview(file) {
                ...
                reader.onload = function (e) {
                    ...
                    toastr.info('New verification photo selected. Click "Update" to save.');
                };
            ...
            $('#btn_remove_image').on('click', function (e) {
                ...
                if (hasOriginalImage) {
                    ...
                    $('#image_badge').html('<i class="ri-check-line me-1 text-success"></i> Current Photo');
