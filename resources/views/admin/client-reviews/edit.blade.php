            ...
            <div class="row">
                ...
                <div class="col-lg-4 col-12">
                    <div class="row g-3">
                        ...
                        {{-- Client Photo Dropzone with Current/Staged Preview --}}
                        <div class="col-12">
                            <div class="card table-card">
                                <div class="card-header table-header">
                                ...
                                <div class="card-body custom-form p-3">
                                    <input type="file" id="client_photo" name="client_photo" class="d-none"
                                        accept="image/png,image/jpeg,image/webp,image/jpg">

                                    <div id="photo_dropzone" class="dropzone-box">
                                        <div id="photo_empty"
                                            class="{{ $review->hasMedia('client_photo') ? 'd-none' : '' }}">
                                            <div class="dropzone-icon">
                                                <i class="ri-user-smile-line"></i>
                                            </div>
                                            <p class="fw-bold text-dark mb-1" style="font-size: 13px;">Drop client photo
                                                here</p>
                                        ...
                                        <div id="photo_preview_box"
                                            class="{{ $review->hasMedia('client_photo') ? '' : 'd-none' }}">
                                            <div class="position-relative rounded-circle overflow-hidden mx-auto mb-2 border"
                                                style="width: 120px; height: 120px; background: #f8fafc;">
                                                <img id="photo_preview_img" src="{{ $review->client_photo_url }}"
                                                    alt="{{ $review->client_name }}"
                                                    onerror="this.onerror=null;this.src='{{ asset('admin/assets/images/default.jpg') }}';"
                                                    style="width: 100%; height: 100%; object-fit: cover;">
                                                <button type="button"
                                                    class="preview-remove-btn {{ $review->hasMedia('client_photo') ? 'd-none' : '' }}"
                                                    id="btn_remove_photo" title="Cancel selected image">
                                                    <i class="ri-close-line"></i>
                                                </button>
                                            </div>
                                            <div class="d-flex align-items-center justify-content-between px-1">
                                                <div class="text-start text-truncate me-2">
                                                    <span id="photo_preview_name"
                                                        class="fw-semibold text-dark d-block text-truncate"
                                                        style="font-size: 12px;">{{ $review->getFirstMedia('client_photo')?->file_name ?? $review->client_name }}</span>
                                                    <span id="photo_preview_size" class="text-muted"
                                                        style="font-size: 11px;">{{ $review->getFirstMedia('client_photo')?->readable_size ?? 'Active avatar' }}</span>
                                                </div>
...
@push('custom-script')
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var originalPhotoSrc = "{{ $review->client_photo_url }}";
            var originalPhotoName = "{{ $review->getFirstMedia('client_photo')?->file_name ?? $review->client_name }}";
            var originalPhotoSize = "{{ $review->getFirstMedia('client_photo')?->readable_size ?? 'Active avatar' }}";
            var hasOriginalImage = {{ $review->hasMedia('client_photo') ? 'true' : 'false' }};
            ...
            function renderPreview(file) {
                ...
                reader.onload = function (e) {
                    ...
                    toastr.info('New client photo selected. Click "Update Review" to save.');
                };
