                ...
                <div class="card table-card mb-4">
                    <div class="card-header table-header">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <a href="{{ route('client-reviews.index') }}" class="add-new" style="background-color: #f1f5f9; color: #334155; border: 1px solid #e2e8f0;">
                    ...
                    <div class="card-body custom-form p-4">
                        {{-- Client Hero Header --}}
                        <div class="d-flex flex-wrap align-items-center justify-content-between p-3 rounded mb-4" style="background: #f8fafc; border: 1px solid #e2e8f0;">
                            <div class="d-flex align-items-center gap-3">
                                <div class="rounded-circle overflow-hidden d-flex align-items-center justify-content-center border" style="width: 72px; height: 72px; background: #ffffff;">
                                    @if($review->client_photo_url)
                                        <img src="{{ $review->client_photo_url }}" alt="{{ $review->client_name }}" style="width: 100%; height: 100%; object-fit: cover;">
                                    @else
                                ...
                                <div>
                                    <h4 class="fw-bold text-dark mb-1" style="font-size: 19px;">{{ $review->client_name }}</h4>
                                    <div class="text-muted" style="font-size: 13px;">
                                        @if($review->designation && $review->company_name)
                                            <span class="text-dark fw-medium">{{ $review->designation }}</span> at <strong class="text-dark">{{ $review->company_name }}</strong>
                                    ...
                                    <div class="d-flex align-items-center gap-1 mt-2">
                                        @for($i = 1; $i <= 5; $i++)
                                            <i class="ri-star-fill {{ $i <= $review->rating ? 'text-warning' : 'text-muted' }}" style="font-size: 14px; {{ $i > $review->rating ? 'opacity: 0.3;' : '' }}"></i>
                                        @endfor
                                        <span class="badge bg-white text-dark border ms-1" style="font-size: 11px; font-weight: 700;">
                            ...
                            <div class="d-flex flex-column align-items-end gap-2 mt-3 mt-sm-0">
                                @if($review->is_published)
                                    <span class="badge" style="background-color: #ecfdf5; color: #065f46; border: 1px solid #a7f3d0; font-size: 12px; padding: 6px 12px; font-weight: 700; border-radius: 4px;">
                                        <i class="ri-checkbox-circle-line me-1"></i> Live on Website
                                    </span>
                                @else
                                    <span class="badge" style="background-color: #f1f5f9; color: #475569; border: 1px solid #cbd5e1; font-size: 12px; padding: 6px 12px; font-weight: 700; border-radius: 4px;">
                                        Hidden / Draft
                                    </span>
                                @endif

                                @if($review->featured)
                                    <span class="badge" style="background-color: #fff7ed; color: #ea580c; border: 1px solid rgba(249,87,22,0.3); font-size: 11.5px; padding: 4px 10px; border-radius: 4px;">
                        ...
                        @if($review->project)
                            <div class="mb-2">
                                ...
                                <div class="p-3 rounded border d-flex flex-wrap align-items-center justify-content-between gap-3" style="background-color: #f8fafc;">
                                    <div class="d-flex align-items-center gap-3">
                                        <div class="rounded overflow-hidden border" style="width: 64px; height: 50px; background: #e2e8f0;">
                                        ...
                                        <div>
                                            <h6 class="fw-bold text-dark mb-1" style="font-size: 14px;">{{ $review->project->title }}</h6>
                                            <span class="text-muted" style="font-size: 12px;">
                                                <i class="ri-map-pin-line me-1"></i> {{ $review->project->location ?: 'Location on file' }}
                                            </span>
                                        </div>
                                    </div>
                                    <a href="{{ route('projects.show', $review->project->id) }}" class="btn btn-sm btn-outline-primary px-3">
                                ...
                                <div class="d-flex flex-column gap-3">
                                    ...
                                    <div class="review-meta-box">
                                        ...
                                        <div class="review-meta-value text-warning">
                                            <i class="ri-star-fill text-warning me-1"></i> {{ $review->rating }}.0 of 5.0 Stars
                                        </div>
                                    </div>
